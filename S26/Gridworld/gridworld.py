...
if __name__ == '__main__':
        ...
        if not opts.manual and opts.agent == 'value':
            if opts.valueSteps:
                for i in range(opts.iters):
                    ...
                    display.displayValues(tempAgent, message = "VALUES AFTER "+str(i)+" ITERATIONS - by Himank")
                    display.pause()

            display.displayValues(a, message = "VALUES AFTER "+str(opts.iters)+" ITERATIONS")
            display.pause()
            display.displayQValues(a, message = "Q-VALUES AFTER "+str(opts.iters)+" ITERATIONS - by Himank")
            display.pause()
    ...
    if not opts.quiet:
        if opts.manual and opts.agent == None:
            ...
            if opts.agent == 'q': displayCallback = lambda state: display.displayQValues(a, state, "CURRENT Q-VALUES - by Himank")
    ...
    if opts.agent == 'q' and not opts.manual:
        try:
            display.displayQValues(a, message = "Q-VALUES AFTER "+str(opts.episodes)+" EPISODES-Himank")
            display.pause()
            display.displayValues(a, message = "VALUES AFTER "+str(opts.episodes)+" EPISODES - by Himank")
            display.pause()
        except KeyboardInterrupt:
            sys.exit(0)
